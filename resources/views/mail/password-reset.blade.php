...
<main id="content" role="main" class="w-full  max-w-lg mx-auto p-6">
    <div class="mt-7 bg-white  rounded-xl shadow-lg dark:bg-gray-800 dark:border-gray-700 border-2 border-indigo-300">
      <div class="p-4 sm:p-7">
        ...
        <div class="mt-5">
		<form action="{{route('post.reset-password')}}" method="POST" class="auth-form"  id="forget_password_form">
        @csrf
			<div class="grid gap-y-4">
              <div>
                <label for="email" class="block text-sm font-bold ml-1 mb-2 dark:text-white">Email address</label>
                <div class="relative">
                  <input type="email" id="email" name="email" class="py-3 px-4 block w-full border-2 border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm" required aria-describedby="email-error" value="{{old('email')}}" id="email">
                </div>
				@if($errors->any('email'))
	                <p class="hidden text-xs text-red-600 mt-2" id="email-error">{{$errors->first('email')}}</p>
				@endif
				</div>
